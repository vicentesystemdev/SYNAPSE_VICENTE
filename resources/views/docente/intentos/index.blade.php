    ...
    <div class="container-fluid">
        <div class="card">
            ...
            <div class="card-body p-0">
                ...
                <div class="p-3">
                    <form action="{{ route('docente.intentos.index') }}" method="GET" class="form-inline">
                        ...
                        @if (request()->has('search') || request()->has('status') || request()->has('evaluacion_id'))
                            <a href="{{ route('docente.intentos.index') }}" class="btn btn-secondary btn-sm ml-2 mb-2">Limpiar</a>
                        @endif
                        {{-- Botón de Exportar a PDF --}}
                        <a href="{{ route('docente.intentos.exportPdf', request()->query()) }}" class="btn btn-danger btn-sm ml-2 mb-2" target="_blank">
                            <i class="fas fa-file-pdf"></i> Exportar PDF
                        </a>

                        {{-- Botones de Ordenamiento --}}
                        <a href="{{ route('docente.intentos.index', array_merge(request()->query(), ['sort_by' => 'id_int', 'sort_direction' => 'asc'])) }}" class="btn btn-default btn-sm ml-2 mb-2" title="Ordenar por ID Ascendente">
                            <i class="fas fa-sort-numeric-up-alt"></i> ID Asc
                        </a>
                        <a href="{{ route('docente.intentos.index', array_merge(request()->query(), ['sort_by' => 'id_int', 'sort_direction' => 'desc'])) }}" class="btn btn-default btn-sm ml-1 mb-2" title="Ordenar por ID Descendente">
                        ...
                        <tbody>
                            @forelse ($intentos as $intento)
                                <tr>
                                    ...
                                    <td>{{ $intento->evaluacion->categoria->nombre_cat ?? 'N/A' }}</td>
                                    ...
                                    <td>
                                        <a href="{{ route('docente.intentos.show', $intento->id_int) }}" class="btn btn-primary btn-xs" title="Ver Detalles">
