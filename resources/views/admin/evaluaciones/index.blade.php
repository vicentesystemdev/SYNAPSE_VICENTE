    ...
    <div class="space-y-6">
        ...
        <div class="rounded-xl border border-white/10 bg-gray-900/85 backdrop-blur-md px-4 py-4 shadow-xl space-y-4">
            <form method="GET" action="{{ route('evaluaciones.index') }}" class="flex flex-wrap items-center gap-3">
                ...
                <a href="{{ route('evaluaciones.index') }}" class="text-xs text-gray-500 hover:text-orange-400 underline transition">
                    Limpiar filtros
                </a>
            </form>
        </div>
                    ...
                    <tbody class="divide-y divide-white/5">
                        @forelse($evaluaciones as $evaluacion)
                            ...
                            <tr class="hover:bg-white/5 transition">
                                ...
                                <td class="px-4 py-3">
                                    <form action="{{ route('evaluaciones.toggle-status', $evaluacion) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        <select name="estado_eval" onchange="this.form.submit()"
