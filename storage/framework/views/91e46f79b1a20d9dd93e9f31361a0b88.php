  ...
  <div class="mx-auto max-w-6xl space-y-10">
    <header class="space-y-2">
      <h1 class="text-3xl font-extrabold tracking-tight text-white">
        <?php echo e($evaluacion->titulo_eval ?? 'Reto Evaluación lógico-matemática'); ?>
    ...
    <div class="grid grid-cols-1 gap-8 md:grid-cols-[minmax(0,2fr)_minmax(0,1.2fr)]">
      <section
        class="relative overflow-hidden rounded-2xl border <?php echo e($yaCompleto ? 'border-emerald-500/60' : 'border-emerald-500/40'); ?> bg-gradient-to-br from-slate-950 via-slate-950 to-emerald-950/30 shadow-[0_0_25px_rgba(16,185,129,0.3)]">
        ...
        <div class="relative px-5 py-6 space-y-6 md:px-7">
            ...
            <div class="rounded-lg border border-emerald-500/50 bg-emerald-500/20 p-6 text-center">
              ...
              <?php if($score): ?>
                <p class="text-xs text-emerald-300/80 font-mono">
                  Puntaje obtenido: <?php echo e(number_format($score->puntaje, 2)); ?> (<?php echo e(number_format($score->porcentaje, 1)); ?>%)
                </p>
              <?php endif; ?>
              <div class="mt-6 flex gap-3 justify-center">
                ...
                <a href="<?php echo e(route('estudiante.evaluaciones.show', $evaluacion->id_eval)); ?>"
          ...
          <div class="rounded-lg border border-slate-800 bg-slate-950/60 p-4">
            <p class="font-mono text-[13px] text-slate-100 leading-relaxed">
                <?php echo e($evaluacion->descripcion_eval ?? 'Analiza los recursos proporcionados y encuentra la respuesta correcta.'); ?>

            </p>
            
            <?php if($evaluacion->archivo_adjunto): ?>
            <div class="mt-4 pt-4 border-t border-slate-800">
                <a href="<?php echo e(asset('storage/' . $evaluacion->archivo_adjunto)); ?>" target="_blank" 
          ...
          <p class="font-mono text-xs text-blue-300">
            ...
            <?php if(session('mensaje')): ?>
                <div class="rounded-lg border <?php echo e(session('status') === 'correcta' ? 'border-emerald-500/40 bg-emerald-500/10 text-emerald-300' : 'border-rose-500/40 bg-rose-500/10 text-rose-300'); ?> px-4 py-3 text-sm font-semibold flex items-center gap-2">
                    <?php if(session('status') === 'correcta'): ?>
                        <span>&#10003;</span>
                    <?php else: ?>
                        <span>&#10007;</span>
                    <?php endif; ?>
            ...
            <?php if($errors->any()): ?>
                <div class="rounded-lg border border-rose-500/50 bg-rose-500/10 px-4 py-3 text-sm text-rose-200 font-semibold flex items-center gap-2">
                    <span>&#10007;</span>
                    <?php echo e($errors->first('flag') ?: $errors->first()); ?>

                </div>
            <?php endif; ?>


          <form action="<?php echo e(route('estudiante.intentos.store', $evaluacion->id_eval ?? $evaluacion->id)); ?>" method="POST" class="space-y-4" id="respuesta-form">
            <?php echo csrf_field(); ?>
            ...
            <div class="space-y-2">
              <input
                ...
                placeholder="Escribe tu respuesta aquí..."
                  class="w-full rounded-xl border <?php echo e($errors->has('flag') ? 'border-rose-500' : 'border-slate-700'); ?> bg-slate-900 px-4 py-3 font-mono text-sm text-slate-200 placeholder-slate-500 shadow-inner focus:border-blue-400 focus:ring-blue-400/30 focus:outline-none transition"
                  value="<?php echo e(old('flag')); ?>" autofocus />
          ...
          <?php if($intentosCount < 5): ?>
            <div class="mt-4 pt-4 border-t border-slate-800 text-center">
                ...
                <form action="<?php echo e(route('estudiante.calibracion.skip')); ?>" method="POST">
                    <?php echo csrf_field(); ?>
                    <input type="hidden" name="evaluacion_id" value="<?php echo e($evaluacion->id_eval); ?>">
                    <button type="submit" 
      ...
      <aside class="space-y-6">
          ...
          <ul class="space-y-2 text-xs">
            <?php $__empty_1 = true; $__currentLoopData = $intentos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $intento): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                ...
                <li class="flex items-center justify-between rounded-lg border border-slate-800 bg-slate-900/70 px-3 py-2">
                  <span class="font-mono text-slate-300">Intento #<?php echo e($intento->nro_intento_int); ?> · <?php echo e(optional($intento->created_at)->format('H:i')); ?></span>
                  <span class="font-mono <?php echo e($resultadoColor); ?>"><?php echo e($resultadoTexto); ?></span>
                  <span class="font-mono text-slate-400"><?php echo e($intento->latencia_seg_int ? $intento->latencia_seg_int . 's' : '--'); ?></span>
                </li>
        ...
        <div class="space-y-1 text-xs text-slate-500 font-mono">
          <a href="<?php echo e(route('estudiante.evaluaciones.show', $evaluacion->id_eval ?? $evaluacion->id)); ?>" class="hover:text-slate-300 transition">
            &gt; Ver detalles del ejercicio
          </a>
          <br>
          <a href="<?php echo e(route('estudiante.evaluaciones.index')); ?>" class="hover:text-slate-300 transition">
...
<?php /**PATH C:\laragon\www\synapse\resources\views/estudiante/evaluaciones/realizar.blade.php ENDPATH**/ ?>